...
def center_toplevel_window(window):
    ...
    y = (screen_height // 2) - (window_height // 2)  # Corrected for actual vertical centering
    window.geometry(f"+{x}+{y}")


class SavingsInputApp(tk.Toplevel):
    """
    A Toplevel window for inputting savings amounts for a specific category.
    Supports both adding new savings and updating existing ones.
    """

    def __init__(self, master=None, category="General Savings", record_id=None, initial_amount=0.0):
        super().__init__(master)
        self.title("Savings Input")
        self.configure(bg=BG_COLOR)
        self.geometry("600x500")  # Original height, as notes are removed
        ...
        self.record_id = record_id  # Store the record_id for update operations
        self.initial_amount = initial_amount  # Store the initial amount for pre-filling

        self.create_widgets()
        center_toplevel_window(self)

    def create_widgets(self):
        """Creates and places all widgets for the savings input window."""
        ...
        main_frame.place(relx=0.5, rely=0.5, anchor="center", width=520,
                         height=450)  # Original height, as notes are removed
    ...
    def on_submit(self):
        """Handles the submission of the savings amount."""
        try:
            ...
            if self.record_id is not None:
                ...
                success = database_manager.update_savings_record(self.record_id, self.category, amount)
                if success:
                    tk.messagebox.showinfo("Success",
                                           f"Savings record (ID: {self.record_id}) updated to ₱{amount:,.2f} for {self.category}!")
                else:
                    tk.messagebox.showerror("Database Error", "Failed to update savings record.")
            else:
                # CORRECTED: Call add_savings_record without the 'notes' parameter
                ...
                if success:
                    ...
                    tk.messagebox.showerror("Database Error", "Failed to save new savings record.")

            if success:
                self.destroy()  # Close the window on successful operation
        except ValueError:
            tk.messagebox.showerror("Invalid Input", "Please enter a valid numeric amount.")


class SelectSavingsToEditWindow(tk.Toplevel):
    """
    A Toplevel window for selecting an existing savings record to edit or delete.
    """

    def __init__(self, master=None):
        super().__init__(master)
        self.title("Select Savings to Edit")
        self.configure(bg=BG_COLOR)
    ...
    def create_widgets(self):
        ...
        header_label = tk.Label(self.records_frame, text="SELECT SAVINGS TO EDIT/DELETE",
    ...
    def load_savings_records(self):
        """Loads savings records from the database and populates the Treeview."""
        for item in self.tree.get_children():
            self.tree.delete(item)

        records = database_manager.get_all_savings()
        for record in records:
            # CORRECTED: Unpack 4 values, as database_manager.get_all_savings() should now return 4 columns.
    ...
    def edit_selected_record(self):
        """Opens the SavingsInputApp to edit the selected record."""
        selected_item = self.tree.selection()
        if not selected_item:
            tk.messagebox.showwarning("No Selection", "Please select a savings record to edit.")
            return
        ...
        category = values[1]
        amount_str = values[2].replace("₱", "").replace(",", "")  # Remove currency symbol and comma
        initial_amount = float(amount_str)

        self.destroy()  # Close this selection window
        # Open SavingsInputApp in edit mode
        SavingsInputApp(self.master, category=category, record_id=record_id, initial_amount=initial_amount)

    def delete_selected_record(self):
        """Deletes the selected savings record from the database."""
        selected_item = self.tree.selection()
        if not selected_item:
            tk.messagebox.showwarning("No Selection", "Please select a savings record to delete.")
            return
        ...
        confirm = tk.messagebox.askyesno(
            "Confirm Delete",
            f"Are you sure you want to delete the savings record for '{category}' (Amount: {amount})?"
        )
        if confirm:
            if database_manager.delete_savings_record(record_id):
                tk.messagebox.showinfo("Success", "Savings record deleted successfully.")
                self.load_savings_records()  # Refresh the list
            else:
                tk.messagebox.showerror("Error", "Failed to delete savings record.")
...
class RoundedFrame(tk.Frame):
    ...
    It is kept for backward compatibility with `SavingsInputApp` potentially using it, if it were to be re-introduced.
    """
...
if __name__ == "__main__":
    ...
    # Directly open the SelectSavingsToEditWindow
    select_window = SelectSavingsToEditWindow(root_app)
    root_app.mainloop()