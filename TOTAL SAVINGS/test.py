...
FONT_MENU = ("Playfair Display", 16) # Increased font size for menu items
    ...
    if script_to_launch:
        try:
            ...
            if logged_in_user_id is not None:
                cmd.append(str(logged_in_user_id)) # Pass the user ID if available

            subprocess.Popen(cmd)
            if item_name != "Total Savings": # Destroy current window unless staying on this page
...
def draw_top_right_header_content(event):
    ...
    if logged_in_username and logged_in_username != "Guest":
        header_text = f"Welcome, {logged_in_username} - Total Savings"
    else:
    ...
    if total_savings_header_label_tk_id:
        ...
        total_savings_header_label_tk_id = top_right_header_canvas.create_window(event.width - 30, event.height / 2, window=total_savings_header_label_tk, anchor="e")
...
def update_pie_chart_section(event=None): # Make event optional
    ...
    pie_chart_section_canvas.create_window(current_width / 2, 40, window=month_combo_widget_ref, anchor="n", tags="month_combo")
    ...
    chart_bottom_y = current_height - 60 # Keep margin at bottom, adjusted to make space for legend
...
def on_recent_transaction_canvas_click(event):
    ...
    for item_id in items_at_click:
        ...
        if "category_link" in tags:
            ...
            print(f"Clicked category: {clicked_category_text}")
            ...
            if os.path.exists(script_to_launch):
                try:
                    ...
                    cmd.append(clicked_category_text) # Pass the clicked category

                    category_child_process_ref = subprocess.Popen(cmd)
                    print(f"Launched {script_to_launch} for category '{clicked_category_text}' and user ID {logged_in_user_id}")
...
def update_recent_transaction_box(event=None): # Make event optional
    ...
    recent_transaction_canvas.coords(recent_transaction_category_combobox_window_id, 30, 60) # Corrected coords for combobox
    ...
    if not filtered_savings:
        ...
        for idx, (id, category, amount, transaction_date_str, notes) in enumerate(filtered_savings):
            ...
            # MODIFIED: Add "category_link" tag to make category text clickable
...
def create_icon_menu(parent, text, command):
    ...
    icon_box_width = 80 # Increased icon box width
    icon_box_height = 80 # Increased icon box height
...
def create_total_savings_app():
    ...
    icon_paths = {
        "Dashboard": os.path.join(current_script_dir, "..", "DASHBOARD", "icons", "dashboard.png"),
        "Total Savings": os.path.join(current_script_dir, "icons", "piggy-bank.png"), # Icon specific to current app
        "Total Expenses": os.path.join(current_script_dir, "..", "TOTAL EXPENSES", "icons", "spending.png"),
        "Profile": os.path.join(current_script_dir, "..", "..", "icons", "user.png")
    }
    ...
    total_savings_amount_lbl_widget_ref = tk.Label(pie_chart_section_canvas, text="Total Savings: Total.",
                                        font=("Georgia", 13), bg=COLOR_BG, fg="black", justify=tk.CENTER) # Changed text and justify
    ...
    # Bind the click handler to the recent_transaction_canvas
