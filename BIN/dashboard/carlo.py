...
main_frame.grid(row=1, column=0, columnspan=2, sticky="nsew", padx=(45, 0), pady=10)  # Remove left padding
...
tk.Label(main_frame, text="Welcome, [username]", font=FONT_WELCOME, bg=COLOR_BG).grid(
    row=0, column=0, columnspan=2, sticky="w", padx=(0, 0), pady=(0, 10))  # Remove left padding

# Left Section - Spending Boxes (Total Expenses & Total Savings)
left_frame = tk.Frame(main_frame, bg=COLOR_BG)
left_frame.grid(row=1, column=0, sticky="nsew", padx=(0, 0))  # Remove right padding
...
for i in range(2):
    ...
    draw_rounded_rect(box_canvas, 0, 0, 440, 300, 20, COLOR_CANVAS_BAR)
