...
# Welcome Text
tk.Label(main_frame, text="Welcome, [username]", font=FONT_WELCOME, bg=COLOR_BG).grid(row=0, column=0, columnspan=2, sticky="w", padx=10, pady=(0, 15))
...
for i in range(2):
    ...
    # Month combobox placed near top
    ...
    # Amount text (move it up a bit for better spacing)
    box_canvas.create_text(175, 110, text="₱ 0.00", font=FONT_AMOUNT, fill="black")

    # Title text (move it up slightly)
    box_canvas.create_text(175, 150, text=box_titles[i], font=FONT_SECTION, fill="black")

    # Bar chart canvas inside the box, width reduced to fit nicely, height adjusted
    bar_canvas = tk.Canvas(box_canvas, width=320, height=130, bg=COLOR_CANVAS_BAR, highlightthickness=0)
    box_canvas.create_window(175, 240, window=bar_canvas)

    # Sample bar heights (replace with your data)
