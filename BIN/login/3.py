...
# ---- Left Panel ----
left_frame = tk.Frame(root, bg="#d9e8d3", width=350)
...
logo_canvas = tk.Canvas(left_frame, width=80, height=70, bg="#d9e8d3", highlightthickness=0)
logo_canvas.place(relx=0.35, y=60)
draw_logo(logo_canvas, 5, 65)
...
right_frame = tk.Frame(root, bg="#fefef5")
right_frame.pack(side="left", fill="both", expand=True)

# Canvas with Grid
canvas = tk.Canvas(right_frame, bg="#fefef5", highlightthickness=0)
...
def draw_grid():
    ...
    spacing = 25  # Space between grid lines
    for x in range(0, w, spacing):
        canvas.create_line(x, 0, x, h, fill="#ccc", tags="grid_line")
    for y in range(0, h, spacing):
        canvas.create_line(0, y, w, y, fill="#ccc", tags="grid_line")
