...
# Group for logo + welcome text (as a template block)
welcome_block = tk.Frame(left_frame, bg="#d9e8d3")
welcome_block.place(relx=0.5, y=100, anchor="n")

# Placeholder image area (replace with actual image if needed)
image_placeholder = tk.Canvas(welcome_block, width=100, height=100, bg="#d9e8d3", highlightthickness=0)
image_placeholder.pack()

tk.Label(welcome_block, text="Welcome to", bg="#d9e8d3", font=("Georgia", 24)).pack()
tk.Label(welcome_block, text="TrackU", bg="#d9e8d3", font=("Georgia", 24, "bold")).pack()
...
# Login Card (placed on top of the rounded canvas)
# card = tk.Frame(canvas, bg="#fce0b0", width=300, height=400)
# card.place(x=0, y=0)
...
# rounded_card.place(x=650 - 300 // 2, y=90)
